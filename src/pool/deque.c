...
static int deque_init(ABT_pool pool, ABT_pool_config config)
{
    ...
    p_data->head_idx = 0;
    p_data->tail_idx = 0;
    ABTI_ticket_lock_create(&p_data->foreign_lock);
...
static int deque_free(ABT_pool pool)
{
    ...
    ABTI_ticket_lock_free(&p_data->foreign_lock);
    ABTU_free(p_data);
